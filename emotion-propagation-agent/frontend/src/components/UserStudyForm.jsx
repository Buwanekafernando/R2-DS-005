...
const emotions = ["joy", "excitement", "trust", "confidence", "curiosity", "optimism", "relief", "admiration", "neutral"]
...
export default function UserStudyForm({ productName, targetEmotion, generatedMessage, onSubmit, loading }) {
  const [participantId, setParticipantId] = useState("")
  const [perceivedEmotion, setPerceivedEmotion] = useState(targetEmotion || "neutral")
  ...
  const canSubmit = useMemo(() => {
    return Boolean(participantId.trim() && productName && targetEmotion && generatedMessage)
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="text-sm font-semibold text-slate-900">User Study Evaluation</div>
      <div className="mt-1 text-sm text-slate-600">Rate the generated message from 1 (low) to 5 (high). Responses are saved to CSV.</div>

      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div>
          <label className="text-sm font-medium text-slate-800">Participant ID</label>
          <input
        ...
        <div>
          <label className="text-sm font-medium text-slate-800">Perceived Emotion</label>
          <select className={selectClass()} value={perceivedEmotion} onChange={(e) => setPerceivedEmotion(e.target.value)}>
            {emotions.map((e) => (
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        {Object.keys(ratings).map((key) => (
          <div key={key} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            <div className="text-sm font-semibold text-slate-900">{fieldLabel(key)}</div>
              ...
              {scales.map((n) => (
                <button
                  ...
                  className={[
                    ...
                    ratings[key] === n ? "border-blue-500 bg-blue-600 text-white" : "border-slate-200 bg-white text-slate-800 hover:bg-slate-50",
                  ].join(" ")}
                >
                  {n}
                </button>
      ...
      <div className="mt-4">
        <label className="text-sm font-medium text-slate-800">Comments (optional)</label>
        <textarea
      ...
      {!canSubmit ? (
        <div className="mt-2 text-xs text-slate-500">Generate a message first, then enter a Participant ID to submit.</div>
      ) : null}
    </form>
  )
}
