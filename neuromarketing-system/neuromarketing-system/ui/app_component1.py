...
# Q6 is now a 1–5 Likert scale from consumer-purchase.csv
# Label shown to user, number sent to API
CULTURE_OPTIONS = [
    "1 — Strongly Disagree",
    "2 — Disagree",
    "3 — Neutral",
...
#consumer profile
        ...
        with d1:
            ...
            st.markdown('<span class="field-label">Age Range</span>',
                        unsafe_allow_html=True)
            d_age = st.selectbox("age", AGE_OPTIONS, index=0, key="d_age")
        ...
        with d2:
            ...
            st.markdown(
                '<span class="field-label">Cultural Influence</span>'
                '<span style="font-size:10px;color:var(--text-muted);'
                'margin-left:6px">(1=Strongly Disagree · 5=Strongly Agree)</span>',
                unsafe_allow_html=True
            )
            d_cult_label = st.selectbox("cult", CULTURE_OPTIONS, index=2,
                                        key="d_cult")
            # Extract the numeric value "1"–"5" from the label
            d_cult = d_cult_label[0]
...
#batch mode
    ...
    if run_batch and batch_input.strip():
        ...
        with st.spinner(f"Analyzing {len(payload)} products…"):
            try:
                response = requests.post(
                    f"{API_URL}/component1/batch-analyze",
                    json    = payload,
                    timeout = 120
                )
                if response.status_code == 200:
