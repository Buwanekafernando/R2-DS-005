...
from src.component1.generator import (
...
# Project root = three levels up from src/component1/agent.py
...
class DualSystemAgent:
    ...
    def _encode_demographics(self,
        ...
        # environment_enc — from district, unchanged
        urban_districts = {
        ...
        # occupation_enc — updated for new employment status values
        occ_map = {
            'student':                        0.7,
        ...
        # spending_enc — updated for new income brackets
        spend_map = {
            ...
            # Old brackets kept for backward compatibility
        ...
        # culture_enc — Q6 is now a 1–5 Likert scale
        try:
            score = float(str(culture_influence).strip())
            culture_enc = (score - 1.0) / 4.0   # normalize to 0–1
        except (ValueError, TypeError):
            cv = str(culture_influence).strip().lower()
            culture_enc = (
                1.0 if 'strongly' in cv or 'yes' in cv else
                0.0 if 'no' in cv else
                0.5
            )
