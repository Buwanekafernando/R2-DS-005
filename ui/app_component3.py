...
from src.component3.pain_point_extractor import extract_pain_points_detailed, get_extractor
from src.component3.review_reader import get_product_pain_profile
...
with col_left:
    ...
    with st.container():
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # dataset file now lives at project_root/dataset/sample_products.json
        sample_file = os.path.join(BASE_DIR, "dataset", "sample_products.json")
        if os.path.exists(sample_file):
            with open(sample_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            
            # Step 1: Select Category (use fixed set matching frontend)
            fixed_categories = [
                "Apparel",
                "Baby",
                "Beauty",
            ...
            # default to Apparel (index 0)
            selected_cat = st.selectbox("Select Market Category", fixed_categories, index=0)
            ...
            p_name = st.text_input("Product Name", "Luxury Handbag", label_visibility="collapsed")
        ...
        p_desc = st.text_area("Product Description", "Authentic leather, handcrafted for a professional look.", label_visibility="collapsed")
