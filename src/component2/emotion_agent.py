...
def build_marketing_prompt(
    ...
    if base_copy:
        # Component 1 (Dual-System Reasoning Agent) has already produced a
        # cognitively-aligned marketing message. Component 2's job here is
        # to INFUSE that existing message with the target emotion, not
        # write an unrelated one from scratch.
        return (
            "You are a world-class marketing copywriter. Brands hire you because your "
            "copy makes people feel something in a single glance and act on it.\n\n"
            "You are given an EXISTING marketing message, already crafted to match this "
            "product's cognitive decision-making profile (System 1 emotional or System 2 "
            "rational). Your job is to REWRITE it so the target emotion comes through "
            "strongly, while preserving its core selling point and claims.\n\n"
            f'EXISTING MESSAGE: "{base_copy}"\n\n'
            f"YOUR GOAL: rewrite this into ONE short marketing message that makes the "
            f"reader feel {target_emotion} so strongly it comes through in every line, "
            "while keeping the same underlying product claim and selling point.\n\n"
            f"HOW TO HIT {target_emotion.upper()}:\n"
            f"- Convey: {signals['convey']}.\n"
            f"- Never drift into: {signals['avoid']}.\n"
            "- Show the feeling through concrete scenes, sensory detail, and word choice \u2014 "
            "do not name the emotion itself.\n\n"
            "NON-NEGOTIABLE RULES:\n"
            "- 35 to 60 words, one tight paragraph.\n"
            "- No bullet points, no hashtags, no emojis, no quotation marks.\n"
            "- Every sentence must earn its place; cut anything flat or generic.\n"
            "- Make no false or unverifiable claims.\n"
            "- Speak directly to the target audience, in their world.\n"
            "- Return ONLY the rewritten marketing message \u2014 no preamble, no explanation.\n"
            f"{correction}"
            "\nPRODUCT BRIEF:\n"
            f"- Name: {product_name}\n"
            f"- Category: {category}\n"
            ...
            f"Rewrite the message now, and make {target_emotion} unmistakable."
        )

    return (
        "You are a world-class marketing copywriter. Brands hire you because your "
        "copy makes people feel something in a single glance and act on it.\n\n"
        f"YOUR GOAL: write ONE short marketing message that makes the reader feel {target_emotion} "
        "so strongly it comes through in every line.\n\n"
        f"HOW TO HIT {target_emotion.upper()}:\n"
        f"- Convey: {signals['convey']}.\n"
        f"- Never drift into: {signals['avoid']}.\n"
        "- Show the feeling through concrete scenes, sensory detail, and word choice \u2014 "
        ...
        "- Make no false or unverifiable claims.\n"
        "- Speak directly to the target audience, in their world.\n"
        "- Return ONLY the marketing message \u2014 no preamble, no explanation.\n"
        f"{correction}"
        "\nPRODUCT BRIEF:\n"
        f"- Name: {product_name}\n"
        f"- Category: {category}\n"
        f"- Audience: {target_audience}\n"
        f"- Key features: {feature_text}\n\n"
        f"Write the message now, and make {target_emotion} unmistakable."
    )
